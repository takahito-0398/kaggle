...
import keras
# from keras_applications.resnext import EfficientNetB7
...
train_dir = "./train_3channel_type2"
test_dir = "./test_3channel_type2"
model_dir = "./models"
INPUT_SHAPE = (27, 128, 3)
...
def roc_auc(y_true, y_pred):
    func = tf.py_function(roc_auc_score, (y_true, y_pred), tf.double)
    return func
# classes = ['1','2','3','4','5']
...
def setModel(input_shape):
    ...
    main_model = EfficientNetB7(include_top=False, weights='imagenet',input_tensor=input_tensor)
    ...
    model.compile(loss='categorical_crossentropy',optimizer=optimizers.SGD(lr=1e-3, momentum=0.9),metrics=['accuracy', roc_auc])
...
if __name__ == '__main__':
    ...
    for epoch in range(N_EPOCHS):
        ...
        for X_batch, Y_batch in get_batch(X_train, y_train, 25):
    ...
    p_model = f"./models/{MODEL_NAME}_3channel_type2_{N_EPOCHS}epochs.h5"
    model.save(p_model)
    ...
    ax3.plot(x, TNs, label='TN', linestyle = "solid")
    ax3.plot(x, FPs, label='FP', linestyle = "dotted")
    ax3.plot(x, FNs, label='FN', linestyle = "dotted")
    ax3.plot(x, TPs, label='TP', linestyle = "solid")
